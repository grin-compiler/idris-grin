...
spec = do
  ...
  modes <- runIO $ do
    ...
    pure $ maybe [NonOptimisedEval, OptimisedEval, Compiled] (const [OptimisedEval]) env
  ...
  describe "Idris and Grin matches for:" $ forM_ modes $ \mode -> describe (show mode) $ do
    ...
    xit "TDD 04 - 01 Data Types" $ timed $ idris mode 120 "test/tdd/chapter04/01_DataTypes.idr"
    ...
    xit "TDD 11 - 01 Exercises" $ timed $ idris mode 60 "test/tdd/chapter11/01_Exercises.idr"
    ...
    -- REASON: Shifting operation doesn't produce right values. Root cause: different Int representation.
    xit "TDD 11 - 01 ShiftTest" $ timed $ idris mode 60 "test/tdd/chapter11/01_ShiftTest.idr"

    it "TDD 11 - 02 InfiniteProcesses" $ idrisWithStdin mode 60 "test/tdd/chapter11/02_InfiniteProcesses.idr" $
      unlines [ "1", "2", "3", "4", "5" ]

    -- REASON: Shifting operation doesn't produce right values. Root cause: different Int representation.
    xit "TDD 11 - 02 TotalArithQuiz" $ idrisWithStdin mode 60 "test/tdd/chapter11/02_TotalArithQuizTest.idr" $
    ...
    xit "TDD 11 - 03 ArithCmdDo" $ idrisWithStdin mode 60 "test/tdd/chapter11/03_ArithCmdDoTest.idr" $
      unlines [ "1", "2", "3", "4", "5" ]
    ...
    -- REASON: Random number generator based on shifting
    xit "TDD 12 - 06 ArithState"     $ idrisWithStdin mode 60 "test/tdd/chapter12/06_ArithState.idr" $
